...
function rssrtvr_lite_strip_specific_tags($html, $tagsToRemove) {
    if (strlen(trim($html))) {
        ...
        return rssrtvr_lite_html_cleanup($dom->saveHTML());
    }
        ...
        if ($do_transform_image) {
            ...
            if ($result) {
                ...
                if ($default_file_name != $file_name) {
                    if (rssrtvr_lite_compare_files($default_image_path, $image_path)) {
...
function rssrtvr_lite_unpack_content($post, $packed_content) {
    ...
    if ('' !== trim($parts[0] ?? '')) {
        $post['post_title'] = rssrtvr_lite_strip_tags($parts[0]);
    }
...
function rssrtvr_lite_get_youtube_video($keyword) {
    ...
    foreach ($items as $item) {
        $page = rssrtvr_lite_file_get_contents('https://www.youtube.com/watch?v=' . $item);
...
function rssrtvr_lite_options_menu() {
    if (isset($_POST['submit_options']) && check_admin_referer('rssrtvr_lite_general_settings')) {
        ...
        if (intval($_POST[RSSRTVR_LITE_PC_INTERVAL] ?? 0) * 60 >= get_option(RSSRTVR_LITE_MAX_EXEC_TIME)) {
            $pseudo_cron_interval = intval($_POST[RSSRTVR_LITE_PC_INTERVAL]);
        } else {
            $pseudo_cron_interval = max(2 * RSSRTVR_LITE_MIN_UPDATE_TIME, round(get_option(RSSRTVR_LITE_MAX_EXEC_TIME) / 60));
        }

        if (update_option(RSSRTVR_LITE_RSS_PULL_MODE, rssrtvr_lite_sanitize_user_input(wp_unslash($_POST[RSSRTVR_LITE_RSS_PULL_MODE])))) {
            wp_clear_scheduled_hook('rssrtvr_lite_update_by_wp_cron');
        }

        if (update_option(RSSRTVR_LITE_PC_INTERVAL, $pseudo_cron_interval)) {
            wp_clear_scheduled_hook('rssrtvr_lite_update_by_wp_cron');
        }
...
?>
    <div class="wrap">
        ...
        <div class="metabox-holder postbox-container">
            <form method="post" action="<?php echo esc_url(rssrtvr_lite_REQUEST_URI()); ?>" name="general_settings">
                <div class="section" style="display:block">
                        ...
                        <tr>
                            ...
                            <td>
                                <select
                                    ...
                                    echo '<option ' . ((get_option(RSSRTVR_LITE_RSS_PULL_MODE) === 'cron') ? 'selected ' : '') . 'value="cron">Cron job or manually</option>';
                                    ?>
                                </select>
                    ...
                    <?php wp_nonce_field('rssrtvr_lite_general_settings'); ?>
                    <br>
...
function rssrtvr_lite_accounts_menu() {
    ...
    if (isset($_POST['modify_accounts']) && check_admin_referer('rssrtvr_lite_accounts')) {
        ...
        foreach ($lite_api_keys as $key) {
            if (isset($_POST[$key])) {
                $accounts[$key] = trim(rssrtvr_lite_sanitize_user_input($_POST[$key]));
            }
        }
...
?>
    <div class="wrap">
        ...
        <br>
        <form method="post" action="<?php echo esc_url(rssrtvr_lite_REQUEST_URI()); ?>">
            ...
            <?php wp_nonce_field('rssrtvr_lite_accounts'); ?>
            <br>
            <div style="text-align:left;">
...
function rssrtvr_lite_set_feed_options($options) {
    ...
    if (!empty($result['interval'])) {
        $result['interval'] = max(RSSRTVR_LITE_MIN_UPDATE_TIME, intval($result['interval']));
    }
...
?>
    <div class="wrap">
        ...
        <?php
        if (
            isset($_GET['edit-feed-id']) && isset($_GET['_wpnonce']) && wp_verify_nonce(sanitize_text_field(wp_unslash($_GET['_wpnonce'])), 'rssrtvr_lite_xml_syndicator')
        ) {
            ...
            if (isset($rssrtvr_lite->feeds[$feed_id]['options'])) {
                ...
                $source = rssrtvr_lite_fixurl($rssrtvr_lite->feeds[$feed_id]['url']);
                $rssrtvr_lite->feedPreview($source, true);
                $rssrtvr_lite->showSettings(true, $rssrtvr_lite->feeds[$feed_id]['options']);
            }
        } elseif (isset($_POST['update_feed_settings']) && check_admin_referer('rssrtvr_lite_xml_syndicator')) {
            ...
            if (isset($_POST['url'])) {
                $new_url = rssrtvr_lite_sanitize_user_input($_POST['url']);
            ...
            $rssrtvr_lite->feeds[(int) $_POST['feed_id']]['options'] = rssrtvr_lite_set_feed_options($rssrtvr_lite->feeds[(int) $_POST['feed_id']]['options']);
            update_option(RSSRTVR_LITE_SYNDICATED_FEEDS, $rssrtvr_lite->feeds);
            $rssrtvr_lite->showMainPage(false);
        } elseif (isset($_POST['check_for_updates']) && check_admin_referer('rssrtvr_lite_xml_syndicator')) {
            ...
            $rssrtvr_lite->showMainPage(false);
        } elseif (isset($_POST['apply_settings_to_selected_feeds']) && check_admin_referer('rssrtvr_lite_xml_syndicator')) {
            $new_options = array_map('sanitize_text_field', wp_unslash($_POST));

            $rssrtvr_lite->modifyFeeds($new_options);
            $rssrtvr_lite->showMainPage(false);
        } elseif (isset($_POST['delete_feeds']) && isset($_POST['feed_ids']) && check_admin_referer('rssrtvr_lite_xml_syndicator')) {
            $rssrtvr_lite->deleteFeeds(array_map('absint', (array) $_POST['feed_ids']), false, true);
            $rssrtvr_lite->showMainPage(false);
        } elseif (isset($_POST['shuffle_update_time']) && isset($_POST['feed_ids']) && check_admin_referer('rssrtvr_lite_xml_syndicator')) {
            $rssrtvr_lite->shuffleUpdateTimes(array_map('absint', (array) $_POST['feed_ids']));
            $rssrtvr_lite->showMainPage(false);
        } elseif (isset($_POST['delete_posts']) && isset($_POST['feed_ids']) && check_admin_referer('rssrtvr_lite_xml_syndicator')) {
            $rssrtvr_lite->deleteFeeds(array_map('absint', (array) $_POST['feed_ids']), true, false);
            $rssrtvr_lite->showMainPage(false);
        } elseif (isset($_POST['delete_feeds_and_posts']) && isset($_POST['feed_ids']) && check_admin_referer('rssrtvr_lite_xml_syndicator')) {
            ...
            if (strlen(trim($source)) && $rssrtvr_lite->feedPreview(rssrtvr_lite_fixurl($source), false)) {
        ...
        } elseif (isset($_POST['syndicate_feed']) && check_admin_referer('rssrtvr_lite_xml_syndicator')) {
            ...
            update_option(RSSRTVR_LITE_SYNDICATED_FEEDS, $rssrtvr_lite->feeds);
            $rssrtvr_lite->showMainPage(false);
        } elseif (isset($_POST['update_default_settings']) && check_admin_referer('rssrtvr_lite_xml_syndicator')) {
            if (intval($_POST['date_min']) > intval($_POST['date_max'])) {
                $_POST['date_min'] = $_POST['date_max'];
            }
        ...
        } elseif (isset($_POST['modify_selected_feeds']) && check_admin_referer('rssrtvr_lite_xml_syndicator')) {
            if (isset($_POST['feed_ids'])) {
...
class RSSRtvr_LITE_Syndicator {
    ...
    function __construct() {
        ...
        // phpcs:ignore WordPress.DB.DirectDatabaseQuery.NoCaching
        // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery
        $option_value = $wpdb->get_var($wpdb->prepare("SELECT option_value FROM {$wpdb->prefix}options WHERE option_name = %s LIMIT 1", RSSRTVR_LITE_FEEDS_UPDATED));
        if ($option_value) {
        ...
        // phpcs:ignore WordPress.DB.DirectDatabaseQuery.NoCaching
        // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery
        $option_value = $wpdb->get_var($wpdb->prepare("SELECT option_value FROM {$wpdb->prefix}options WHERE option_name = %s LIMIT 1", RSSRTVR_LITE_SYNDICATED_FEEDS));
        if ($option_value) {
        ...
        if (! empty(array_filter($this->feeds))) {
            ...
            update_option(RSSRTVR_LITE_SYNDICATED_FEEDS, $this->feeds);
        }
    ...
    function parse_placeholders($content) {
        ...
        if (strpos($content, '%post_content_notags%') !== false) {
            $content = str_replace('%post_content_notags%', rssrtvr_lite_strip_tags($this->post['post_content']), $content);
        }
        ...
        if (strpos($content, '%post_excerpt_notags%') !== false) {
            $content = str_replace('%post_excerpt_notags%', rssrtvr_lite_strip_tags($this->post['post_excerpt']), $content);
        }
        ...
        $content = preg_replace_callback(
            '/%post_content\[(.*?)\]%/s',
            function ($matches) {
                return rssrtvr_lite_shorten_html(trim($this->post['post_content']), intval($matches[1]));
            },
            $content
        );
        ...
        $content = preg_replace_callback(
            ...
            function ($matches) {
                return rssrtvr_lite_shorten_string_by_words(rssrtvr_lite_strip_tags($this->post['post_content']), intval($matches[1]));
            },
            $content
        );
        ...
        $content = preg_replace_callback(
            '/%post_excerpt\[(.*?)\]%/s',
            function ($matches) {
                return rssrtvr_lite_shorten_html($this->post['post_excerpt'], intval($matches[1]));
            },
            $content
        );
        ...
        $content = preg_replace_callback(
            ...
            function ($matches) {
                return rssrtvr_lite_shorten_string_by_words(rssrtvr_lite_strip_tags($this->post['post_excerpt']), intval($matches[1]));
            },
            $content
        );
    ...
    function parseFeed($feed_url) {
        ...
        if (strpos($content, 'PK') === 0) {
            $content = rssrtvr_lite_unzip($content);
        ...
        if ($this->insideitem >= 0) {
            ...
            switch ($this->tag) {
                ...
                case 'CATEGORY':
                    ...
                    break;
                case 'TAGS':
                    $tags = explode(',', trim(rssrtvr_lite_fix_white_spaces($data)));
                    foreach ($tags as $tag) {
            ...
            $this->feed_title .= rssrtvr_lite_fix_white_spaces($data);
    ...
    function filterPost() {
        ...
        if (
            ...
            mb_strlen(rssrtvr_lite_strip_tags($this->post['post_content'])) < $this->current_feed['options']['filter_post_longer']
        ) {
        ...
        if (
            ...
            mb_strlen(rssrtvr_lite_strip_tags($this->post['post_content'])) > $this->current_feed['options']['filter_post_shorter']
        ) {
    ...
    function insertPost() {
        ...
        if (! mb_strlen(trim($this->post['post_title']))) {
            if (mb_strlen($this->post['post_excerpt'])) {
                ...
                $text = rssrtvr_lite_strip_tags($this->post['post_content']);
            }
        ...
        if (strpos($this->post['link'], '.youtube.com/') !== false || strpos($this->post['link'], '//youtube.com/') !== false) {
            ...
            $content = rssrtvr_lite_file_get_contents($this->post['link'], false, '', 'self', RSSRTVR_LITE_CURL_USER_AGENT);
        ...
        if (trim($this->current_feed['options']['strip_tags']) !== '') {
            $this->post['post_content'] = rssrtvr_lite_strip_specific_tags($this->post['post_content'], explode(',', $this->current_feed['options']['strip_tags']));
        ...
        if ($this->current_feed['options']['translator'] !== 'none') {
            ...
            $packed_content = rssrtvr_lite_pack_conetnt($this->post, true);

            if ($this->current_feed['options']['translator'] === 'yandex_translate') {
                $translated = rssrtvr_lite_yandex_translate(get_option(RSSRTVR_LITE_ACCOUNTS)['yandex_api_key'], $packed_content, $this->current_feed['options']['yandex_translation_dir']);
            } elseif ($this->current_feed['options']['translator'] === 'google_translate') {
                $translated = rssrtvr_lite_google_translate(get_option(RSSRTVR_LITE_ACCOUNTS)['google_api_key'], $packed_content, $this->current_feed['options']['google_translation_source'], $this->current_feed['options']['google_translation_target']);
            } elseif ($this->current_feed['options']['translator'] === 'deepl_translate') {
                $translated = rssrtvr_lite_deepl_translate(get_option(RSSRTVR_LITE_ACCOUNTS)['deepl_api_key'], $packed_content, $this->current_feed['options']['deepl_translation_target'], ($this->current_feed['options']['deepl_use_api_free'] === 'on'));
            }
            ...
            $this->post = rssrtvr_lite_unpack_content($this->post, $translated);
        }
        ...
        if ($this->current_feed['options']['auto_tags'] === 'on') {
            ...
            foreach ($terms as $term) {
                if (preg_match('/\b' . esc_html(preg_quote($term->name, '/')) . '\b/isu', ' ' . rssrtvr_lite_strip_tags($this->post['post_title'] . ' ' . $this->post['post_content'] . ' ' . $this->post['post_excerpt'] . ' '))) {
        ...
        if ($this->current_feed['options']['remove_emojis_from_slugs'] === 'on') {
            $this->post['post_name'] = rssrtvr_lite_remove_emojis($this->post['post_name']);
        }
        ...
        if (isset($post_thumb_src)) {
            if ($this->current_feed['options']['use_fifu'] === 'on') {
                ...
                $image_url = $media_urls[] = rssrtvr_lite_save_image($post_thumb_src, $this->post['post_title']);
            }
        }
        ...
        if (count($rssrtvr_lite_images_to_attach)) {
            $attach_ids = [];
            foreach ($rssrtvr_lite_images_to_attach as $image) {
                if (strlen(trim($image['url']))) {
                    if ($image['title'] !== '') {
                        $attach_id = rssrtvr_lite_add_image_to_library($image['url'], $image['title'], $post_id);
                    } else {
                        $attach_id = rssrtvr_lite_add_image_to_library($image['url'], stripslashes($this->post['post_title']), $post_id);
                    }
                    if ($attach_id) {
        ...
        if ($this->current_feed['options']['set_thumbnail'] !== 'no_thumb') {
            ...
            if (! empty($image_url)) {
                if ($this->current_feed['options']['use_fifu'] !== 'on') {
                    ...
                    if (rssrtvr_lite_is_binary($image_url)) {
        ...
        if (get_post_meta($post_id, '_rssrtvr_rss_source', true) === '') {
            $this->log('Unable to set the rssrtvr_lite_rss_source filed');
            $this->log('The post will not be added' . PHP_EOL);
            $this->delete_post_media($post_id);
            wp_delete_post($post_id, true);
            return;
    ...
    function showSettings($islocal, $settings, $change_selected = false) {
        ...
        <form name="feed_settings" action="<?php echo esc_url(preg_replace('/\&edit-feed-id\=[0-9]+/', '', rssrtvr_lite_REQUEST_URI())); ?>" method="post">
            ...
            if (isset($_POST['modify_selected_feeds']) && check_admin_referer('rssrtvr_lite_xml_syndicator')) {
    ...
    ?>
        <div class="metabox-holder postbox-container" style="width:100%;">
            ...
            <form id="syndycated_feeds" action="<?php echo esc_url(rssrtvr_lite_REQUEST_URI()); ?>" method="post">
                <?php
                if (count($this->feeds) > 0) {
                    ...
                    for ($i = 0; $i < count($this->feeds); $i++) {
                        ...
                        $feed_item .= '<td><a href="' . $this->feeds[$i]['url'] . '" target="_blank">' . rssrtvr_lite_chop_str(htmlspecialchars($this->feeds[$i]['url']), 100) . '</a></td>';
...
<?php
    ...
    function enqueue_scripts($hook) {
        if (
            ...
            $hook !== 'rss-retriever-lite_page_rssrtvr_lite_accounts' &&
            $hook !== 'rss-retriever-lite_page_rssrtvr_lite_syndicator_log'
        ) {
            return;
        }
        ...
        wp_localize_script(
            ...
            'rssrtvr_lite_vars',
            [
                'rss_pull_mode' => get_option(RSSRTVR_LITE_RSS_PULL_MODE),
                'post_type_map' => $map,
            ]
        );
    }
...
if (! is_admin()) {
    ...
    if ((time() - get_option(RSSRTVR_LITE_POST_LIFE_CHECK_DATE)) > RSSRTVR_LITE_POST_LIFE_CHECK_PERIOD) {
        // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery
        $post_ids = $wpdb->get_col(
            $wpdb->prepare(
                "SELECT post_id 
        ...
        update_option(RSSRTVR_LITE_POST_LIFE_CHECK_DATE, time());
    }
} else {
    add_action('admin_bar_menu', 'rssrtvr_lite_add_admin_menu_item');
    add_action('admin_enqueue_scripts', [$rssrtvr_lite, 'enqueue_scripts']);
    add_filter('plugin_action_links_' . plugin_basename(__FILE__), 'rssrtvr_lite_plugins_action_link');

    if (get_option(RSSRTVR_LITE_KEEP_IMAGES) !== 'on') {
...
function rssrtvr_lite_main_menu() {
    add_menu_page(
        ...
        'manage_options',
        'rssretriever_lite',
        'rssrtvr_lite_xml_syndicator_menu',
        'dashicons-rss'
    );
    ...
    add_submenu_page(
        ...
        esc_html__('Settings', 'rss-retriever-lite'),
        'manage_options',
        'rssrtvr_lite_general_settings',
        'rssrtvr_lite_options_menu'
    );
    ...
    add_submenu_page(
        ...
        esc_html__('Syndicator log', 'rss-retriever-lite'),
        'manage_options',
        'rssrtvr_lite_syndicator_log',
        'rssrtvr_lite_syndicator_log_menu'
    );
...
if (is_admin()) {
    add_action('admin_menu', 'rssrtvr_lite_main_menu');
}
?>